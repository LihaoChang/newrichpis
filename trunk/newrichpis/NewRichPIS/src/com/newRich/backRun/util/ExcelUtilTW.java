...
public class ExcelUtilTW {
	public static SimpleDateFormat sdf = new SimpleDateFormat("yyyy_MM_dd");

	public static HSSFWorkbook makeUpExcelFile(ArrayList<Stock> newStockBeanList) {

		Date thisDate = new Date();
		String dateString = sdf.format(thisDate);
		HSSFWorkbook workbook = new HSSFWorkbook();
		// HSSFWorkbook workbook = wb0;
		HSSFSheet sheet1 = workbook.createSheet("A");

		makeUpExcelSheet(workbook, newStockBeanList, sheet1);
		return workbook;
	}

	public static void makeUpExcelSheet(HSSFWorkbook workbook, List list, HSSFSheet sheet) {
		String checkNum = "0.01";// +-倍率
		// sheet.addMergedRegion(new Region(0, (short) 1, 0, (short) 10));// 跨一行三列
		// 凍結
		...
		HSSFCell row1_cell2 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell2.setCellValue(new HSSFRichTextString("Title"));
		row1_cell2.setCellStyle(headtitelstyle);

		HSSFCell row1_cell3 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell3.setCellValue(new HSSFRichTextString("PrefixedTicker"));
		row1_cell3.setCellStyle(headtitelstyle);

		HSSFCell row1_cell31 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell31.setCellValue(new HSSFRichTextString("Now Price"));
		row1_cell31.setCellStyle(headtitelstyle);

		HSSFCell row1_cell4 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell4.setCellValue(new HSSFRichTextString("淨獲利"));
		row1_cell4.setCellStyle(headtitelstyle);

		HSSFCell row1_cell5 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell5.setCellValue(new HSSFRichTextString("淨獲利成長"));
		row1_cell5.setCellStyle(headtitelstyle);

		HSSFCell row1_cell51 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell51.setCellValue(new HSSFRichTextString("淨毛利"));
		row1_cell51.setCellStyle(headtitelstyle);

		HSSFCell row1_cell52 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell52.setCellValue(new HSSFRichTextString("負債/資產比例"));
		row1_cell52.setCellStyle(headtitelstyle);

		HSSFCell row1_cell211 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell211.setCellValue(new HSSFRichTextString("帳面股價"));
		row1_cell211.setCellStyle(headtitelstyle);

		HSSFCell row1_cell21 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell21.setCellValue(new HSSFRichTextString("現金配股"));
		row1_cell21.setCellStyle(headtitelstyle);

		HSSFCell row1_cell14 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell14.setCellValue(new HSSFRichTextString("ROE(淨值報酬率)"));
		row1_cell14.setCellStyle(headtitelstyle);

		HSSFCell row1_cell15 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell15.setCellValue(new HSSFRichTextString("ROA(總資產報酬率)"));
		row1_cell15.setCellStyle(headtitelstyle);

		HSSFCell row1_cell6 = header_Row1.createCell((short) (cellIndex++),
				HSSFCell.CELL_TYPE_STRING);
		row1_cell6.setCellValue(new HSSFRichTextString("股息百分比"));
		...
		tempStyle_numberic_black.setVerticalAlignment(tempStyle.getVerticalAlignment());
		tempStyle_numberic_black.setFillPattern(tempStyle.getFillPattern());
		tempStyle_numberic_black.setFillForegroundColor(tempStyle.getFillForegroundColor());
		tempStyle_numberic_black.setFont(tempStyle.getFont(workbook));
		...
		if (list != null && list.size() > 0) {
			for (int w = 0; w < list.size(); w++) {
				...
				row_n_cell.setCellStyle(tempStyle);
	...
	public static String getCellFillForegroundColor(Cell cell) {
		...
		if (cell != null) {
			...
			// System.out.println("getFillForegroundColor -----------" +
