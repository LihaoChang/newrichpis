...
public class StockDao extends BaseDao{
	...
	/**
     * 讀取所有資料，分頁
     * from ex: 使用者點選第二頁，傳入的參數就是10
     * pageShowSize 每頁要秀的筆數
     * @return List 
     */
    ...
    private static final String UPDATE_SQL = 
    	" UPDATE stock SET " 
    	+ "title = ?, nowPrice = ?, url = ?, sharesTraded = ?, "
		+ "prefixedTicker = ?, netIncome = ?, netIncomeGrowth = ?, netMargin = ?, debtEquity = ?, "
		+ "bookValuePerShare = ?, cashPerShare = ?, roe = ?, roa = ?, dividend = ?, "
		+ "reScheduleDate = ?, createDate = ?, updateDate = ?, sector = ? "
		+ "where stockCode = ?";
	...
	public int update(StockVO vo) {
		final Object[] params = new Object[] {
			vo.getTitle(), vo.getNowPrice(), vo.getUrl(), vo.getSharesTraded(),
	        vo.getPrefixedTicker(), vo.getNetIncome(), vo.getNetIncomeGrowth(), vo.getNetMargin(), vo.getDebtEquity(),
	        vo.getBookValuePerShare(), vo.getCashPerShare(), vo.getRoe(), vo.getRoa(), vo.getDividend(), 
	        vo.getReScheduleDate(), vo.getCreateDate(), vo.getUpdateDate(), vo.getSector(), 
	        vo.getStockCode()
     ...
     *  
     */
    private static final String SELECT_SECTOR_SQL = 
     ...
     * @return List 
     */
	private static final String SELECT_stock_SQL = 
		    "SELECT * "
			+ "FROM stock ";
	...
	public Stock findByStockCodeToStock(String StockCode) {
    	...
    	return (Stock) jdbcTemplate.query(SELECT_StockCode_SQL, params, new RowMapperResultSetExtractor(new StockMapper2()));
    }
     ...
     * @return List 
     */
	private static String SELECT_StockFindGood2DB_SQL = 
     ...
     *  
     */
    private static final String UPDATE_StockSector2DB_SQL = 
    	" UPDATE stock SET " 
    	+ "updateDate = ?, sector = ? "
		+ "where stockCode = ?";
