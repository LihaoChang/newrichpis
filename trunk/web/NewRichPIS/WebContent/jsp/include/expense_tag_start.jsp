...
<div class="mainSubTitle">
	<bean:message key="expense_item"/>
</div>

	<html:form action="/Expense_add">	
  <table width="90%" border="0">
    ...
    <%
      if(String.valueOf(request.getAttribute("expense_add")).equals("repeat")){
    %>
    <bean:message key="add_error"/><bean:message key="item_name_error2"/>
    <%}%>
    <%
      if(String.valueOf(request.getAttribute("expense_add")).equals("error")){
    %>
    ...
    <%}%>
    <%
      if(String.valueOf(request.getAttribute("expense_update")).equals("error")){
    %>
    <bean:message key="update_error"/>
    <%}%>
		...
		<%
		//System.out.println("-expense_add----error-"+request.getAttribute("expense_add"));
		System.out.println(" ");
		%>
