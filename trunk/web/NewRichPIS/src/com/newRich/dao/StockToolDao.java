...
public class StockToolDao extends BaseDao{
	...
	public List<StockTool> findByFromEnd(StockToolForm formVO, int from, int pageShowSize) {
		String SELECT_formVO_SQL = 
		    "SELECT * FROM Stock_Tool "
			+ "where 1=1 ";
	...
	public StockTool findById(String id) {
    	...
    	return (StockTool) jdbcTemplate.query(SELECT_BY_ID_SQL, params, new RowMapperResultSetExtractor(new StockToolMapper()));
    }
    ...
    private static final String INSERT_SQL =
    		" INSERT INTO Stock_Tool "
			+ " (id, userid, name, url, remark, updateDate) "
			+ " VALUES "
			+ " (?, ?, ?, ?, ?, ?) ";
    public int insert(StockTool vo) {
        final Object[] params = new Object[] {
        		vo.getId(), vo.getUserid(), vo.getName(), vo.getUrl(), vo.getRemark(), vo.getUpdateDate()
        };
    ...
    private static final String UPDATE_SQL = 
    	" UPDATE Stock_Tool SET " 
    	+ "userid = ?, name = ?, url = ?, remark = ?, updateDate = ? where id = ? ";
    	
	public int update(StockTool vo) {
		final Object[] params = new Object[] {
				vo.getUserid(), vo.getName(), vo.getUrl(), vo.getRemark(), vo.getUpdateDate(), vo.getId()
		};
